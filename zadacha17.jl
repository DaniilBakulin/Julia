...
function move_forward!(rob, side)      ##робот движется в выбранном направлении если он дошел до восточной стены или барьера тогда поставить маркер
    c = 0
...
function bypass!(rob, side) ## робот обходит преграды двигаясб вниз пока не будет свободно затем возвращается вниз 
    counter = 0
    counter_1 = 0
