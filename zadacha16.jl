...
function move_forward!(rob, side)   ## робот движется в выбранном направлении ,напрвдение выбирается переменной side        
    c = 0
...
function move_through_barrier!(rob, side)  ##робот огибает перегородки назад в сторону движения вниз на 2 если прошли если нет тогда назад в сторону и повторяем пока не пройдем 
    counter = 0
    counter_1 = 0
...
function move_to_corner!(rob)      ## робот идет в северозападный угол 
    counters = []
