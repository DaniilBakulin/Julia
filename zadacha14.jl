...
function move_if_possible!(rob::Robot, now_side::HorizonSide)::Bool ## функция возвращает значение true если нет барьера 
    ...
    while isborder(rob, now_side) ## если нет барьера тогда робот идет влево обходят барьер спереди иначе мы вернемся назад вправо 
