...
function bypass(rob, dir) ## эта функция нужна чтобы пройти к началу выполнения функции cntsegments
...
function countSegments(rob) ## функция проходит змейкой по полю и считывает все перегородки выводит сумму перегородок
    ans = 0
...
function processRow(rob, side, toCheckSide) ## функция проверяет является ли перегородка перегородкой или это верхняя стенка возвращает +1 если это перегородка 
    cnt = 0
    flag = false
